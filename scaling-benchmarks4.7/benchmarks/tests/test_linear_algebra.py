...
@pytest.fixture(params = np.logspace(1, 7, 7, base=2, dtype=int).tolist())
...
def change_dtype(A, dtype):
    ...
    if dtype == np:
        ...
        return qt.Qobj(A)
#Supported dtypes
dtype_list = [np, sc, qt.Qobj]
dtype_ids = ['numpy', 'scipy_csr', 'Qobj']
...
def test_linear_algebra_unary(benchmark, matrix, dtype, density, get_operation, request):
    ...
    try:
        ...
        result = benchmark(operation, A, density, 100)
